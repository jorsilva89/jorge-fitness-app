...
const supabase = createClient(
  "https://awbppjbizeoxhfguqdge.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF3YnBwamJpemVveGhmZ3VxZGdlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE2NTYyMTAsImV4cCI6MjA5NzIzMjIxMH0.nNODSDOG2sco5Oxtaw-sTkUhxW6ejgLDC0celowYN-Q"
);
...
// ═══════════ STORAGE (Supabase) ═══════════
const load=async(k)=>{try{const{data}=await supabase.from('user_data').select('value').eq('key',k).single();return data?data.value:null}catch(e){return null}};
const save=async(k,v)=>{try{await supabase.from('user_data').upsert({key:k,value:v,updated_at:new Date().toISOString()},{onConflict:'key'})}catch(e){}};

// ═══════════ EXERCISEDB API (via proxy) ═══════════
const gifCache={};
async function fetchGif(exId){
  ...
  if(!term)return null;
  try{
    const r=await fetch(`/api/exercise-gif?name=${encodeURIComponent(term)}`);
    const d=await r.json();
    if(d&&d.gifUrl){gifCache[exId]=d.gifUrl;await save('gif_'+exId,d.gifUrl);return d.gifUrl;}
  }catch(e){}
  return null;
}
...
function Active({week,dayIdx,exHistory,prs,onFinish}){
  ...
  const[notes,setNotes]=useState({});const[showInfo,setShowInfo]=useState(false);
  const tRef=useRef(null);const audioRef=useRef(null);

  // Request notification permission on mount
  useEffect(()=>{if('Notification' in window&&Notification.permission==='default')Notification.requestPermission()},[]);
  useEffect(()=>{const iv=setInterval(()=>tick(t=>t+1),60000);return()=>clearInterval(iv)},[]);

  // Timer with background notification support
  useEffect(()=>{
    if(!showTimer||timer<=0){
      if(showTimer&&timer<=0){
        // Timer finished — notify even if screen off
        try{navigator.vibrate?.([300,100,300,100,300])}catch(e){}
        if('Notification' in window&&Notification.permission==='granted'){try{new Notification('Descanso terminado',{body:'Siguiente serie lista',icon:'🏋️',tag:'rest-timer',requireInteraction:true})}catch(e){}}
      }
      return;
    }
    tRef.current=setTimeout(()=>setTimer(s=>s-1),1000);
  ...
  const toggleDone=(si)=>{
    const s=sets[exIdx][si];
    if(s.done){
      // Undo — allow editing again
      setSets(p=>({...p,[exIdx]:p[exIdx].map((s,i)=>i===si?{...s,done:false,isPR:false}:s)}));
    } else {
  ...
  const finish=()=>{
    const newExH={};const newPRs={};const allNotes={...notes};
    exs.forEach((e,i)=>{const[eid2]=e;const sd=sets[i]||[];const done=sd.filter(s=>s.done&&s.kg);
      if(done.length>0){newExH[eid2]={date:new Date().toISOString(),sets:done.map(s=>({reps:s.reps,kg:s.kg})),notes:allNotes[i]||''};
    ...
    onFinish({week,dayIdx,duration:elapsed,exercisesDone:totalDone,totalExercises:exs.length,newExH,newPRs,dayName:d_name,notes:allNotes});
  };
  ...
  if(showTimer&&timer>0)return<div style={{...sty.page,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100vh',gap:14,padding:'40px 20px'}}>
    ...
    <div style={{fontSize:10,color:C.t3,marginTop:4}}>Puedes apagar la pantalla — vibra al terminar</div>
    <div style={{display:'flex',gap:10,marginTop:8}}>
      <button onClick={()=>setTimer(s=>s+30)} style={{...sty.bts,padding:'10px 20px'}}>+30s</button>
      <button onClick={()=>{setShowTimer(false);setTimer(0)}} style={{...sty.btn,width:'auto',padding:'10px 26px'}}>Listo →</button>
    </div>
  </div>;
  ...
  <div style={sty.wrap}>
    ...
    <div style={{...sty.card,padding:0,overflow:'hidden',marginBottom:10}}>
      ...
      {exSets.map((s,si)=>{
        ...
        return<div key={si} style={{display:'grid',gridTemplateColumns:'24px 54px 1fr 1fr 26px',padding:'8px 10px',borderBottom:si<exSets.length-1?`0.5px solid ${C.br}`:'none',background:s.done?(s.isPR?'rgba(255,215,0,0.06)':C.okd):'transparent',alignItems:'center',gap:4}}>
          ...
          <input type="number" min="0" placeholder={eReps.replace(/\D/g,'')||'—'} value={s.reps} onChange={e=>updateSet(si,'reps',e.target.value)} style={{...sty.inp,padding:'5px 6px',textAlign:'center',fontSize:13}}/>
          <input type="number" min="0" step="2.5" placeholder={p?.kg||'kg'} value={s.kg} onChange={e=>updateSet(si,'kg',e.target.value)} style={{...sty.inp,padding:'5px 6px',textAlign:'center',fontSize:13}}/>
          <div style={{display:'flex',justifyContent:'center'}}>
            <button onClick={()=>toggleDone(si)} style={{width:24,height:24,borderRadius:6,border:`1px solid ${s.done?'transparent':C.brm}`,background:s.done?(s.isPR?'rgba(255,215,0,0.15)':'rgba(24,160,112,0.15)'):'transparent',cursor:'pointer',color:s.done?(s.isPR?C.gold:C.ok):C.t3,fontSize:s.done?14:11,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'inherit'}}>{s.done?'✓':'○'}</button>
          </div>
        </div>})}
    </div>
    ...
    <div style={{...sty.card,marginBottom:10}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
        <span style={{fontSize:11,fontWeight:600,color:C.t2}}>Notas y técnica</span>
        <div style={{display:'flex',gap:6}}>
          <YTButton exId={eid}/>
          <button onClick={()=>setShowInfo(!showInfo)} style={{...sty.bts,padding:'4px 8px',fontSize:11}}>{showInfo?'Ocultar GIF':'Ver GIF'}</button>
        </div>
      </div>
      {showInfo&&<div style={{marginBottom:8}}><ExGif exId={eid}/></div>}
      <textarea placeholder="Ej: Subir peso próxima vez, cuidar forma en última serie..." value={notes[exIdx]||''} onChange={e=>setNotes(p=>({...p,[exIdx]:e.target.value}))} rows={2} style={{...sty.inp,resize:'vertical',fontSize:12,lineHeight:'1.4'}}/>
    </div>

    <div style={{display:'flex',gap:8,marginBottom:14}}>
      <button onClick={()=>{setExIdx(i=>Math.max(0,i-1));setShowTimer(false);setShowInfo(false)}} disabled={exIdx===0} style={{...sty.bts,flex:1,opacity:exIdx===0?0.25:1}}>← Anterior</button>
      {exIdx<exs.length-1?<button onClick={()=>{setExIdx(i=>i+1);setShowTimer(false);setShowInfo(false)}} style={{...sty.btn,flex:2}}>Siguiente →</button>:<button onClick={finish} style={{...sty.btn,flex:2,background:C.ok}}>Finalizar ✓</button>}
    </div>
    <div style={{display:'flex',gap:4,justifyContent:'center',flexWrap:'wrap'}}>
      {exs.map((_,i)=>{const sd=sets[i]||[];const done=sd.every(s=>s.done);return<button key={i} onClick={()=>{setExIdx(i);setShowTimer(false);setShowInfo(false)}} style={{width:8,height:8,borderRadius:'50%',border:'none',cursor:'pointer',padding:0,background:done?C.ok:i===exIdx?C.ac:C.s3,flexShrink:0}}/>})}
    </div>
  </div></div>
}
...
function Progress({metrics,history,exHistory,prs,measures,saveMetrics,saveMeasures}){
  ...
  const exportCSV=()=>{
    let csv='HISTORIAL DE ENTRENAMIENTO - Jorge Silva\n\n';
    // Workout history
    csv+='SESIONES\nFecha,Semana,Dia,Duracion(min),Ejercicios\n';
    history.forEach(h=>{csv+=`${new Date(h.date).toLocaleDateString('es-CO')},${h.week},${h.name||''},${h.duration||0},${h.exercisesDone||0}/${h.totalExercises||0}\n`});
    csv+='\nEJERCICIOS - HISTORIAL DE PESOS\nFecha,Ejercicio,Sets(kg x reps),Notas\n';
    Object.entries(exHistory).forEach(([id,hist])=>{
      (hist||[]).forEach(h=>{
        const setsStr=(h.sets||[]).map(s=>`${s.kg}kg x${s.reps}`).join(' | ');
        csv+=`${new Date(h.date).toLocaleDateString('es-CO')},${exNames[id]||id},"${setsStr}",${h.notes||''}\n`;
      });
    });
    csv+='\nRECORDS PERSONALES\nEjercicio,Peso(kg),Reps,Fecha\n';
    Object.entries(prs).forEach(([id,pr])=>{csv+=`${exNames[id]||id},${pr.kg},${pr.reps},${new Date(pr.date).toLocaleDateString('es-CO')}\n`});
    csv+='\nPESO CORPORAL\nFecha,Peso(kg)\n';
    metrics.forEach(m=>{csv+=`${new Date(m.date).toLocaleDateString('es-CO')},${m.weight}\n`});
    csv+='\nMEDIDAS CORPORALES\nFecha,Cintura,Pecho,BrazoD,BrazoI,Muslo,Cadera,Cuello\n';
    measures.forEach(m=>{csv+=`${new Date(m.date).toLocaleDateString('es-CO')},${m.cintura||''},${m.pecho||''},${m.brazoD||''},${m.brazoI||''},${m.muslo||''},${m.cadera||''},${m.cuello||''}\n`});
    const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});
    const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`jorge_fitness_${new Date().toISOString().split('T')[0]}.csv`;a.click();URL.revokeObjectURL(url);
  };

  const TABS=[{id:'peso',l:'Peso'},{id:'medidas',l:'Medidas'},{id:'ejercicios',l:'Ejercicios'},{id:'records',l:'PRs'}];

  return<div style={sty.page}><div style={sty.hdr}><div style={{fontSize:14,fontWeight:700}}>Progreso</div><button onClick={exportCSV} style={{...sty.bts,padding:'5px 10px',fontSize:11}}>📥 Exportar</button></div>
